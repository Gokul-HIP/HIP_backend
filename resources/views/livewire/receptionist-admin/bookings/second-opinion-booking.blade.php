<div class="space-y-6">
    ...
    <div>
        <h1 class="text-2xl font-bold text-slate-900">Second Opinion Bookings</h1>
        <p class="text-sm text-slate-500 mt-1">Second opinion requests for your hospital</p>
    </div>
        ...
        <table class="w-full text-left text-sm">
            <thead style="background:#EBF5FB;">
                <tr>
                    ...
                    <th class="px-4 py-3 text-xs font-bold uppercase" style="color:var(--button-color);">Speciality</th>
                    <th class="px-4 py-3 text-xs font-bold uppercase" style="color:var(--button-color);">Preferred Date</th>
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse ($bookings as $booking)
                    ...
                    @endphp
                    <tr class="hover:bg-slate-50" wire:key="rec-so-{{ $booking->id }}">
                        <td class="px-4 py-3 font-medium" style="color:var(--button-color);">SO-{{ str_pad($booking->id, 5, '0', STR_PAD_LEFT) }}</td>
                        ...
                        <td class="px-4 py-3">{{ $booking->speciality?->name ?: '—' }}</td>
                        <td class="px-4 py-3">
                            <div>{{ $booking->preferred_date?->format('d M Y') ?: '—' }}</div>
                        ...
                        <td class="px-4 py-3">
                            <div class="inline-block">
                                <button type="button" class="rec-bk-btn w-8 h-8 inline-flex items-center justify-center rounded-lg text-slate-400"
                                    onclick="recBkToggle(event,'recSoMenu{{ $booking->id }}')">
                                    <i class="fa-solid fa-ellipsis-vertical"></i>
                                </button>
                                <div id="recSoMenu{{ $booking->id }}" class="rec-bk-menu" style="display:none;">
                                    <a href="{{ route('receptionist.second-opinion-bookings.appointment-details', $booking->id) }}" onclick="recBkCloseAll()" class="rec-bk-item">
                ...
                @empty
                    <tr>
                        <td colspan="10" class="px-6 py-10 text-center text-gray-500">No second opinion bookings found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
