<div id="subscriptions-index-root">
    ...
    <div class="flex items-center justify-between px-6 py-4 flex-wrap gap-3">
        ...
        <a href="{{ route('cashier.manage-subscriptions.create') }}"
           class="inline-flex items-center gap-2 px-5 py-2.5 text-white font-semibold text-sm rounded-lg shadow-md shadow-slate-400/30" style="background: var(--button-color); hover:background: var(--button-hover);">
    ...
    @if($showRenewModal && $renewSubscriptionId)
        <div class="fixed inset-0 z-[10000] flex items-center justify-center p-4">
            ...
            <div class="relative bg-white rounded-xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto z-10">
                @livewire('cashier.subscriptions.renew', ['subscriptionId' => $renewSubscriptionId], key('renew-'.$renewSubscriptionId))
            </div>
        </div>
    @endif
</div>
