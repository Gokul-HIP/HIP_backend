{{-- livewire/admin/bookings/procedure-bookings.blade.php --}}
<div class="space-y-6">
    ...
    <style>
        ...
        .hip-proc-item:last-child { border-bottom: none; }
        .hip-proc-item:hover { background: #f0f9ff; color: #1A9FD4; }
        .hip-proc-item:hover svg { color: #1A9FD4; }
        .hip-proc-btn { background: transparent; border: none; cursor: pointer; }
    </style>

    {{-- ── Page Header ── --}}
    <div>
        <h1 class="text-2xl font-bold text-slate-800">Procedure Bookings – Appointment Form Submissions</h1>
    ...
    <div style="display:grid; grid-template-columns:repeat(5,1fr); gap:1rem;">
        ...
        <div class="bg-white rounded-xl border border-slate-100 shadow-sm flex items-center gap-3" style="padding:1.25rem 1rem;">
            ...
            <div class="min-w-0">
                <p class="font-medium text-slate-500 uppercase leading-tight" style="font-size:10px; letter-spacing:.04em;">Total Procedures</p>
        ...
        <div class="bg-white rounded-xl border border-slate-100 shadow-sm flex items-center gap-3" style="padding:1.25rem 1rem;">
            ...
            <div class="min-w-0">
                <p class="font-medium text-slate-500 uppercase leading-tight" style="font-size:10px; letter-spacing:.04em;">Upcoming (7 days)</p>
        ...
        <div class="flex gap-3 flex-wrap">
            ...
            <div class="relative" x-data="{ open: false }">
                <button type="button" @click="open = !open"
                    ...
                    {{ $procedureFilter === 'all' ? 'All Procedures' : ($availableProcedures->firstWhere('id', (int) $procedureFilter)?->name ?? 'All Procedures') }}
                ...
                <div x-show="open" x-cloak @click.away="open = false" x-transition
                    ...
                    <button type="button" wire:click="$set('procedureFilter','all')" @click="open=false" class="hip-proc-item">All Procedures</button>
                    @foreach ($availableProcedures as $proc)
                        <button type="button" wire:click="$set('procedureFilter','{{ $proc->id }}')" @click="open=false" class="hip-proc-item">{{ $proc->name }}</button>
                    @endforeach
                </div>
            </div>
            ...
            <div class="relative" x-data="{ open: false }">
                ...
                <div x-show="open" x-cloak @click.away="open = false" x-transition
                    ...
                    <button type="button" wire:click="$set('statusFilter','all')" @click="open=false" class="hip-proc-item">All Status</button>
                    <button type="button" wire:click="$set('statusFilter','pending')" @click="open=false" class="hip-proc-item">Pending</button>
                    <button type="button" wire:click="$set('statusFilter','confirmed')" @click="open=false" class="hip-proc-item">Confirmed</button>
                    <button type="button" wire:click="$set('statusFilter','completed')" @click="open=false" class="hip-proc-item">Completed</button>
                    <button type="button" wire:click="$set('statusFilter','cancelled')" @click="open=false" class="hip-proc-item">Cancelled</button>
                </div>
            </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead style="background:#EBF5FB;">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-xs font-bold uppercase tracking-wider whitespace-nowrap" style="color:#1A9FD4;">Procedure</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50 transition-colors" wire:key="proc-booking-{{ $booking['id'] }}">

                        <td class="px-6 py-4 text-sm font-medium text-slate-500 whitespace-nowrap">{{ $booking['apt_id'] }}</td>

                        <td class="px-6 py-4 text-sm font-semibold text-slate-800 whitespace-nowrap">{{ $booking['name'] }}</td>

                        <td class="px-6 py-4">
                            <p class="text-sm text-slate-800">{{ $booking['member_name'] }}</p>
                            <p class="text-xs font-medium text-slate-400 uppercase tracking-tight">{{ $booking['member_hip_id'] }}</p>
                        </td>

                        <td class="px-6 py-4 text-sm text-slate-600 whitespace-nowrap">{{ $booking['mobile_number'] }}</td>

                        <td class="px-6 py-4">
                            <p class="text-sm text-slate-800 whitespace-nowrap">{{ $booking['hospital_name'] }}</p>
                            <p class="text-xs font-medium text-slate-400 uppercase tracking-tight">{{ $booking['hospital_meta'] }}</p>
                        </td>

                        <td class="px-6 py-4">
                            <p class="text-sm text-slate-800 whitespace-nowrap">{{ $booking['procedure_name'] }}</p>
                            <p class="text-xs font-medium text-slate-400 uppercase tracking-tight">{{ $booking['procedure_meta'] }}</p>
                        </td>

                        <td class="px-6 py-4">
                            <p class="text-sm text-slate-800 whitespace-nowrap">{{ $booking['booking_date'] }}</p>
                        ...
                        <td class="px-6 py-4">
                            <div class="inline-block">
                                <button type="button"
                                    class="hip-proc-btn w-8 h-8 inline-flex items-center justify-center rounded-lg text-slate-400 transition-colors"
                                    style="hover:color:#1A9FD4;"
                                    onclick="hipProcToggle(event,'procMenu{{ $booking['id'] }}')">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                                        <circle cx="5" cy="12" r="1.5"/><circle cx="12" cy="12" r="1.5"/><circle cx="19" cy="12" r="1.5"/>
                                    </svg>
                                </button>
                                <div id="procMenu{{ $booking['id'] }}" class="hip-proc-menu" style="display:none;">
                                    <a href="{{ route('healthcare.procedure.booking.appointment-details', $booking['id']) }}" onclick="hipProcCloseAll()" class="hip-proc-item">
                                    ...
                                    <button type="button" wire:click="openDeleteBookingModal({{ $booking['id'] }})" onclick="hipProcCloseAll()" class="hip-proc-item" style="color:#ef4444;">
                                    ...
                                    <button type="button" wire:click="openUpdateStatusModal({{ $booking['id'] }})" onclick="hipProcCloseAll()" class="hip-proc-item">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 flex-shrink-0" style="color:#1A9FD4;" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    ...
                    <tr>
                        <td colspan="9" class="px-6 py-12 text-center">
                            <p class="text-sm font-medium text-slate-800">No procedure bookings found.</p>
    ...
    <script>
        ...
        function hipProcCloseAll() {
            document.querySelectorAll('.hip-proc-menu').forEach(function(m) { m.style.display = 'none'; });
        }
        document.addEventListener('click', hipProcCloseAll);
        window.addEventListener('scroll', hipProcCloseAll, true);
        window.addEventListener('resize', hipProcCloseAll);
    </script>
