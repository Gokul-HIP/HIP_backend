...
<div class="space-y-6">
    ...
    {{-- ── Page Header ── --}}
    <div>
        <h1 class="text-2xl font-bold text-slate-800">Second Opinion Bookings</h1>
        <p class="text-sm text-slate-500 mt-1">Manage second opinion consultation requests for your hospitals</p>
    </div>
    ...
    <div style="display:grid; grid-template-columns:repeat(5,1fr); gap:1rem;">
        ...
        <div class="bg-white rounded-xl border border-slate-100 shadow-sm flex items-center gap-3" style="padding:1.25rem 1rem;">
            ...
            <div class="min-w-0">
                <p class="font-medium text-slate-500 uppercase leading-tight" style="font-size:10px; letter-spacing:.04em;">Total Second Opinions</p>
        ...
        <div class="bg-white rounded-xl border border-slate-100 shadow-sm flex items-center gap-3" style="padding:1.25rem 1rem;">
            ...
            <div class="min-w-0">
                <p class="font-medium text-slate-500 uppercase leading-tight" style="font-size:10px; letter-spacing:.04em;">Upcoming (7d)</p>
        ...
        <div class="flex gap-3 flex-wrap">
            ...
            <div class="relative" x-data="{ open: false }">
                ...
                <div x-show="open" x-cloak @click.away="open = false" x-transition
                    ...
                    <button type="button" wire:click="$set('hospitalFilter','all')" @click="open=false" class="hip-doc-bk-item">All Hospitals</button>
                    @foreach ($availableHospitals as $hospital)
                        <button type="button" wire:click="$set('hospitalFilter','{{ $hospital->id }}')" @click="open=false" class="hip-doc-bk-item">{{ $hospital->name }}</button>
                    @endforeach
                </div>
            </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse text-sm">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50 transition-colors" wire:key="so-booking-{{ $booking['id'] }}">

                        <td class="px-6 py-4 font-medium text-slate-500 whitespace-nowrap">{{ $booking['apt_id'] }}</td>

                        <td class="px-6 py-4">
                            <p class="font-semibold text-slate-800 whitespace-nowrap">{{ $booking['patient_name'] }}</p>
                            @if($booking['relationship'])
                                <p class="text-xs text-slate-400">{{ $booking['relationship'] }}</p>
                            @endif
                        </td>
                        ...
                        <td class="px-6 py-4 font-medium text-slate-800 whitespace-nowrap">{{ $booking['hospital_name'] }}</td>

                        <td class="px-6 py-4">
                            <p class="font-medium text-slate-800 whitespace-nowrap">{{ $booking['doctor_name'] }}</p>
                            <p class="text-xs text-slate-400">{{ $booking['speciality_name'] }}</p>
                        </td>

                        <td class="px-6 py-4">
                            <p class="font-medium text-slate-800 whitespace-nowrap">{{ $booking['preferred_date'] }}</p>
                            <p class="text-xs text-slate-400">{{ $booking['preferred_time'] }}</p>
                        </td>
                        ...
                        <td class="px-6 py-4">
                            <div class="inline-block">
                                ...
                                <div id="docBkMenu{{ $booking['id'] }}" class="hip-doc-bk-menu" style="display:none;">
                                    {{-- View --}}
                                    <a href="{{ route('healthcare.second-opinion.booking.appointment-details', $booking['id']) }}" onclick="hipDocBkCloseAll()" class="hip-doc-bk-item">
                    ...
                    <tr>
                        <td colspan="9" class="px-6 py-12 text-center">
                            <p class="text-sm font-medium text-slate-800">No second opinion bookings found.</p>
    ...
    <flux:modal name="delete-second-opinion" class="p-0" wire:close="closeDeleteBookingModal" id="delete-org">
        <div x-data @click.outside="$wire.closeDeleteBookingModal()">
            <div>
                ...
                <h2 class="text-lg font-semibold text-gray-900 mb-2">Delete Second Opinion</h2>
                <p class="text-sm text-gray-500 mb-4 leading-relaxed">Are you sure you want to delete this second opinion booking?</p>
