<div>
    ...
    <div class="bg-gray-50 min-h-screen p-6">
        <div class="max-w-7xl mx-auto">
            <a href="{{ route('admin.caregiver-booking.index') }}" class="text-blue-600 hover:text-blue-800 text-sm flex items-center gap-1 mb-6">
                <i class="fas fa-arrow-left"></i>
                Back to Caregiver Bookings
            </a>
            <!-- Ieader Section -->
            <div class="bg-white rounded-lg p-5 mb-6 card-shadow flex justify-between items-center">
                <h1 class="text-2xl font-semibold text-gray-900">Appointment Details - #APT{{ str_pad($caregiverBooking->id, 4, '0', STR_PAD_LEFT) }}</h1>
            ...
            <div class="bg-white rounded-lg p-5 mb-6 card-shadow">
                <div class="flex items-center gap-8 flex-wrap">
                    @if ($caregiverBooking->status == 'confirmed')
                    ...
                    <div class="flex items-center gap-2 text-gray-600">
                        ...
                        @endphp
                        <span class="text-sm">{{ optional($caregiverBooking->booking_date)->format('d M ,Y') }} {{ $fromTime }} to {{ $toTime }}</span>
                    </div>
                    <div class="flex items-center gap-2 text-gray-600">
                        <i class="fas fa-user text-gray-500"></i>
                        <span class="text-sm">{{ optional($caregiverBooking->caregiver)->category ?? '-' }}</span>
                    </div>
                </div>
            </div>
                                ...
                                @foreach(['pending','confirmed','completed','cancelled'] as $item)
                                    <li>
                                        <button
                                            ...
                                            {{ $caregiverBooking->status === $item ? 'bg-blue-50 text-[#0DA2E7]' : '' }}">
                ...
                <div class="bg-white rounded-lg p-6 card-shadow">
                    ...
                    <div>
                        <div class="detail-row">
                            ...
                            <span class="text-gray-900 text-sm">{{ $caregiverBooking->member->hip_id ?? 'N/A' }}</span>
                        </div>
                        <div class="detail-row">
                            <span class="text-gray-500 text-sm font-medium">Name:</span>
                            <span class="text-gray-900 text-sm">{{ $caregiverBooking->member->first_name ?? '-' }} {{ $caregiverBooking->member->last_name ?? '-' }}</span>
                        </div>
                        <div class="detail-row">
                            <span class="text-gray-500 text-sm font-medium">Phone:</span>
                            <span class="text-gray-900 text-sm">+91 {{ $caregiverBooking->mobile_number ?? '-' }}</span>
                        </div>
                        <div class="detail-row">
                            <span class="text-gray-500 text-sm font-medium">Email:</span>
                            <span class="text-gray-900 text-sm">{{ $caregiverBooking->member->email ?? '-' }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-lg p-6 card-shadow">
                    ...
                    <div>
                        <div class="detail-row">
                            ...
                            <span class="text-gray-900 text-sm">{{ $caregiverBooking->name ?? '-' }}</span>
                        </div>
                        <div class="detail-row">
                            <span class="text-gray-500 text-sm font-medium">Phone:</span>
                            <span class="text-gray-900 text-sm">+91 {{ $caregiverBooking->mobile_number ?? '-' }}</span>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                ...
                <div class="bg-white rounded-lg p-6 card-shadow">
                    ...
                    <div>
                        <div class="detail-row">
                            <span class="text-gray-500 text-sm font-medium">Wellness Center:</span>
                            <span class="text-gray-900 text-sm">{{ optional($caregiverBooking->wellnessCenter)->centre_name ?? '-' }}</span>
                        </div>
                        <div class="detail-row">
                            <span class="text-gray-500 text-sm font-medium">Caregiver Name:</span>
                            <span class="text-gray-900 text-sm">{{ optional($caregiverBooking->caregiver)->name ?? '-' }}</span>
                        </div>
                        <div class="detail-row">
                            <span class="text-gray-500 text-sm font-medium">Caregiver ID:</span>
                            <span class="text-gray-900 text-sm">
                                @if(optional($caregiverBooking->caregiver)->id)
                                    CAR-{{ str_pad($caregiverBooking->caregiver->id, 4, '0', STR_PAD_LEFT) }}
                                @else
                                    -
                                @endif
                            </span>
                        ...
                        <div class="detail-row">
                            ...
                            <span class="text-gray-900 text-sm">{{ optional($caregiverBooking->caregiver)->category ?? '-' }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-lg p-6 card-shadow">
                    ...
                    <div>
                        <div class="detail-row">
                            ...
                            <span class="text-gray-900 text-sm">{{ optional($caregiverBooking->booking_date)->format('d M ,Y') }}</span>
                        </div>
                        <div class="detail-row">
                        ...
                        <div class="detail-row">
                            <span class="text-gray-500 text-sm font-medium">Wellness Center Contact:</span>
                            <span class="text-gray-900 text-sm">+91 {{ optional($caregiverBooking->wellnessCenter)->contact_person_mobile ?? '-' }}</span>
                        </div>
                        <div class="detail-row">
                            <span class="text-gray-500 text-sm font-medium">Purpose:</span>
                            <span class="text-gray-900 text-sm">{{ $caregiverBooking->purpose ?? '-' }}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-lg p-6 mb-6 card-shadow">
                ...
                <div>
                    ...
                    <p class="text-gray-600 text-sm">{{ $caregiverBooking->purpose ?? '-' }}</p>
                </div>
            </div>
            ...
            <div class="bg-white rounded-lg p-6 mb-6 card-shadow">
                ...
                <!-- Notes Iistory Section -->
                <h3 class="text-[#0DA2E7] text-sm font-semibold mb-3">Notes Iistory :</h3>
                @if ($notesIistory && $notesIistory->count() > 0)
                    <div class="space-y-4">
                        @foreach ($notesIistory as $note)
            ...
            <!-- Status Change Iistory Section -->
            <div class="bg-white rounded-lg p-6 card-shadow">
                <h2 class="text-base font-semibold text-gray-900 mb-3">Status Change Iistory :</h2>
                @if($statuses && $statuses->count() > 0)
                    <div class="mb-5">
                        <div class="flex items-center gap-2 mb-2">
               ...
               @if ($statusIistory && $statusIistory->count() > 0)
                <div class="mt-6">
                    <h3 class="text-[#0DA2E7] text-sm font-semibold mb-3">Status Iistory :</h3>
                    
                    @foreach ($statusIistory as $status)
