<div class="space-y-6" 
    ...
    <!-- OVERVIEW -->
    <div>
        <h2 class="text-2xl font-semibold mb-4 text-gray-900">Procedure Booking Overview</h2>

        <div class="flex gap-4 overflow-x-auto pb-2">

            <div class="bg-white p-4 border border-gray-200 rounded-lg shadow-md min-w-[200px]">
                <p class="text-xs text-gray-500">Total Procedure Bookings</p>
                <p class="text-4xl font-bold mt-1 text-gray-900">{{ $procedureBookings->total() }}</p>
            </div>
        
            <div class="bg-white p-4 border border-gray-200 rounded-lg shadow-md min-w-[200px]">
                <p class="text-xs text-gray-500">Total Pending</p>
                <p class="text-4xl font-bold mt-1 text-gray-900">
                    {{ $procedureBookings->where('status', 'pending')->count() }}
                </p>
            </div>
            ...
            <div class="bg-white p-4 border border-gray-200 rounded-lg shadow-md min-w-[200px]">
                ...
                <p class="text-4xl font-bold mt-1 text-gray-900">
                    {{ $procedureBookings->where('status', 'cancelled')->count() }}
                </p>
            </div>
            ...
            <div class="bg-white p-4 border border-gray-200 rounded-lg shadow-md min-w-[200px]">
                ...
                <p class="text-4xl font-bold mt-1 text-gray-900">
                    {{ $procedureBookings->where('status', 'completed')->count() }}
                </p>
            </div>
        
            <div class="bg-white p-4 border border-gray-200 rounded-lg shadow-md min-w-[260px]">
                <p class="text-xs text-gray-500">
                    Upcoming Procedure Bookings
                    <span class="text-xs">(Next 7 days)</span>
                </p>
                <p class="text-4xl font-bold mt-1 text-gray-900">
                    {{ $procedureBookings->where('booking_date', '<=', now()->addDays(7))->where('booking_date', '>=', now())->count() }}
                </p>
            </div>
        ...
        <div class="flex items-center justify-between mb-6">
            ...
            <div class="flex items-center space-x-3 flex-wrap gap-3">
                ...
                <div class="relative">
                    ...
                    <input
                        type="text"
                        placeholder="Search name, mobile, member, hospital, procedure..."
                ...
                <!-- PROCEDURE DROPDOWN -->
                <div class="relative">
                    <button onclick="toggleFilter('procedureFilter')" class="filter-btn">
                        <i class="fas fa-procedures mr-2 text-gray-700"></i>
                        <span class="filter-label">
                            @if($procedureFilter === 'all')
                                All Procedures
                            @else
                                {{ $availableProcedures->where('id', $procedureFilter)->first()->procedure_name ?? 'All Procedures' }}
                            @endif
                        </span>
                        <i class="fa-solid fa-angle-down w-4 ml-3"></i>
                    </button>

                    <div id="procedureFilter" class="filter-dropdown hidden max-h-96 overflow-y-auto">
                        <ul class="p-2 text-sm text-gray-700 font-medium">
                            <li>
                                <button class="inline-flex items-center w-full p-2 hover:bg-gray-100 rounded"
                                    onclick="selectFilter(this,'procedureFilter')"
                                    wire:click="$set('procedureFilter','all')">
                                    <i class="fas fa-list mr-2 text-gray-700"></i> All Procedures
                                </button>
                            </li>
                            ...
                            <li>
                                <button class="inline-flex items-center w-full p-2 hover:bg-gray-100 rounded"
                                    onclick="selectFilter(this,'procedureFilter')"
                                    wire:click="$set('procedureFilter',{{ $procedure->id }})">
                                    <i class="fas fa-stethoscope mr-2 text-gray-700"></i> {{ $procedure->procedure_name }}
                                </button>
                            </li>
                            @endforeach
                        </ul>
                        ...
                        <ul class="p-2 text-sm text-gray-700 font-medium">
                            ...
                            <li>
                                <button class="inline-flex items-center w-full p-2 hover:bg-gray-100 rounded"
                                    ...
                                    <i class="fas fa-hospital mr-2 text-gray-700"></i> {{ $hospital->name }}
                                </button>
                            </li>
                            @endforeach
                        </ul>
        ...
        <table class="w-full border-collapse table-fixed shadow-md rounded-lg">
            ...
            <thead class="bg-gray-100 border-b">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-sm font-semibold text-gray-700">Procedure</th>
            ...
            <tbody class="divide-y divide-gray-200">
                ...
                @forelse ($procedureBookings as $procedureBooking)
                    <tr class="hover:bg-gray-50">

                        <td class="px-6 py-4 text-sm">
                            APT-{{ str_pad($procedureBooking->id, 4, '0', STR_PAD_LEFT) }}
                        </td>

                        <td class="px-6 py-4 text-sm">
                            {{ $procedureBooking->name }}
                        </td>
                        ...
                        <td class="px-6 py-4 text-sm">
                           +91 {{ $procedureBooking->mobile_number }}
                        </td>
                        ...
                        <td class="px-6 py-4">
                            @if($procedureBooking->procedure)
                                {{ $procedureBooking->procedure->procedure_name }}
                                <br>
                                <span class="text-xs text-gray-500">PRO-{{ str_pad($procedureBooking->procedure->id, 4, '0', STR_PAD_LEFT) }}</span>
                            @else
                                <span class="text-gray-400">-</span>
                            @endif
                        </td>
                        ...
                        <td class="px-6 py-4 text-sm">
                            @if($procedureBooking->booking_date)
                                <div class="font-medium text-gray-800">
                                    {{ $procedureBooking->booking_date->format('M d, Y') }}
                                </div>
                                ...
                                <div class="text-xs text-gray-500 mt-1">
                                    @php
                                        ...
                                        if (is_array($slots) && count($slots) >= 2) {
                                            $timeText = $slots[0] . ' - ' . end($slots);
                                        } elseif (is_array($slots) && count($slots) === 1) {
                                            $timeText = $slots[0];
                                        } else {
                                            $timeText = '-';
                                        }
                        ...
                        <td class="px-6 py-4">
                            <span class="px-3 py-1 rounded-full text-xs font-medium
                                {{ $procedureBooking->status === 'confirmed'
                                    ? 'bg-green-100 text-green-700'
                                    : ($procedureBooking->status === 'cancelled'
                                        ? 'bg-red-100 text-red-700'
                                        : ($procedureBooking->status === 'pending'
                                            ? 'bg-yellow-100 text-yellow-700'
                                            : ($procedureBooking->status === 'completed'
                                                ? 'bg-blue-100 text-blue-700'
                                                : 'bg-gray-100 text-gray-700'))) }}">
                                {{ ucfirst($procedureBooking->status ?? 'pending') }}
                            </span>
                        </td>
                        ...
                        <td class="px-6 py-4">
                            <div class="action-menu-wrapper">
                                ...
                                <button
                                    class="action-btn"
                                    onclick="toggleActionMenu(event,'menu-{{ $procedureBooking->id }}')">
                                    <i class="fa-solid fa-ellipsis-vertical"></i>
                                </button>

                                <div id="menu-{{ $procedureBooking->id }}" class="action-menu hidden">
                                    <ul class="p-2 text-sm text-gray-700 font-medium">
                                        ...
                                        <li>
                                            <button
                                                wire:click="openUpdateStatusModal({{ $procedureBooking->id }})"
                                                class="inline-flex items-center w-full p-2 hover:bg-gray-100 rounded">
                ...
                @empty
                    <tr>
                        <td colspan="9" class="px-6 py-10 text-center text-gray-500">
                            ...
                            <p class="text-lg font-medium text-gray-900">No procedure bookings found</p>
