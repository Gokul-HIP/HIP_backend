<div>
    ...
    <flux:modal name="add-disease-package" class="p-0" wire:close="closeModal">
        <div x-data="{ modalReady: false }"
            x-init="$el.closest('dialog').addEventListener('click', (e) => { if (e.target === e.currentTarget && modalReady) $wire.closeModal(); })"
            @modal-show.window="if ($event.detail.name === 'add-disease-package') { modalReady = false; setTimeout(() => modalReady = true, 300); }">
            <div class="p-4 sm:p-6 bg-gray-100 min-h-[600px]">
                <h2 class="text-xl font-semibold mb-2">Add Disease Package</h2>
                ...
                <form wire:submit.prevent="addDiseasePackage">
                    ...
                    <div class="flex items-center justify-between mt-6">
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            @else
                                <flux:button variant="primary" wire:click="addDiseasePackage" type="submit" class="bg-green-600 text-white">Save Package</flux:button>
                            @endif
                        </div>
                    </div>
                </form>
