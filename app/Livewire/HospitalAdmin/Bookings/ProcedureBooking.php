...
use App\Models\ProcedureBooking as ProcedureBookingModel;
...
class ProcedureBooking extends Component
{
    use WithPagination;
    ...
    public string $procedureFilter = 'all';
    ...
    public function updatingProcedureFilter(): void
    {
        $this->resetPage();
    }
    ...
    {
        $this->reset(['search', 'statusFilter', 'procedureFilter', 'dateFilter']);
        $this->resetPage();
    }
    ...
    protected function findScopedBooking(int $id): ?ProcedureBookingModel
    {
        return ProcedureBookingModel::query()
    ...
    public function deleteBooking(): void
    {
        ...
        ProcedureBookingStatus::query()
            ->where('procedure_booking_id', $booking->id)
            ->delete();
    ...
    public function refreshProcedureBookings(): void
    {
    ...
    public function render()
    {
        ...
        $baseScopedQuery = ProcedureBookingModel::query()
            ->whereIn('hospital_id', $hospitalIds);

        $query = ProcedureBookingModel::query()
            ->with(['member', 'hospital', 'procedure'])
            ->whereIn('hospital_id', $hospitalIds);
        ...
        if ($this->procedureFilter !== 'all') {
            $query->where('procedure_id', (int) $this->procedureFilter);
        }
        ...
        $bookingRows = $query
            ->orderByDesc('id')
            ->paginate(10)
            ->withPath(route('healthcare.procedure.booking'));

        $bookingRows->setCollection(
            $bookingRows->getCollection()->map(function (ProcedureBookingModel $booking) {
                ...
                return [
                    ...
                    'procedure_name' => $booking->procedure?->procedure_name ?: '-',
                    'procedure_meta' => $booking->procedure?->procedure_code ?: 'PROC' . str_pad((string) ($booking->procedure_id ?? 0), 4, '0', STR_PAD_LEFT),
        ...
        $availableProcedures = Procedure::query()
            ->where('organization_id', $organizationId)
            ->whereIn('hospital_id', $hospitalIds)
            ->orderBy('procedure_name')
            ->get(['id', 'procedure_name']);

        $availableProcedures->transform(function (Procedure $procedure) {
            $procedure->name = $procedure->procedure_name;
            return $procedure;
        });

        return view('livewire.hospital-admin.bookings.procedure-booking', [
