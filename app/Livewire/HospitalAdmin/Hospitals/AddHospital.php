...
class AddHospital extends Component
{
    ...
    {
        $this->org_id = Auth::guard('filament')->user()?->organization_id;
        $this->organization = Organization::findOrFail($this->org_id);

        $this->pharmacies = Pharmacy::where('organization_id', $this->org_id)->get();
        $this->diagnosticCenters = Diagnostic::where('organization_id', $this->org_id)->get();
    }
    ...
    {
        return view('livewire.hospital-admin.hospitals.add-hospital');
    }
    ...
    public function addHospital(): void
    {
        ...
        $this->resetInput();
        Flux::modal('add-hospital')->close();
        $this->dispatch('toast', type: 'success', message: 'Hospital ' . $hospitalName . ' added successfully!');
