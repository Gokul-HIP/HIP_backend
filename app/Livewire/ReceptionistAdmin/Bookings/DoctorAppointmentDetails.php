...
class DoctorAppointmentDetails extends Component
{
    ...
    {
        $this->doctorBooking = DoctorBooking::query()
            ->whereIn('hospital_id', $this->scopeService()->hospitalIds('all'))
    ...
    {
        $doctorBooking = $this->scopeService()->findDoctorBooking((int) $this->id);
    ...
    public function render()
    {
        return view('livewire.hospital-admin.bookings.doctor-appointment-details', [
            'backRoute' => route('receptionist.doctor-bookings.index'),
        ]);
    }
}
